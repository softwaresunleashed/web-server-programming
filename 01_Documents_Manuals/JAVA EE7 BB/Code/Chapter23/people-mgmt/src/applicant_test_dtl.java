...
public class applicant_test_dtl extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if( (lDBOpr != null && lDBOpr.length() > 0) && (lDBOpr.equals("call_for_written")) ){ //call_for_written
			action = "people_applicant_select";
		}
		else
		if( (lDBOpr != null && lDBOpr.length() > 0) && (lDBOpr.equals("call")) ){ //call
		...
		if ( action_submit != null || action_edit != null || action_select != null || action_wrtn_dtl_submit != null  ){
			if ( request.getParameter("submit").equals("Submit") ){
				...
				if ( action_submit != null && action_submit.equals("people_applicant_register_submit") ){
			...
			if ( request.getParameter("submit").equals("Submit Detail") ){
				if ( action_submit.equals("people_applicant_wrtn_test_dtl_submit") ){
					System.out.println("in the people_applicant_wrtn_test_dtl_submit ");
					action = "people_applicant_wrtn_test_dtl_submit";
				}
			}
			else
			if ( request.getParameter("submit").equals("Update Detail") ){
				if ( action_submit.equals("people_applicant_wrtn_test_dtl_upd_submit") ){
					System.out.println("in the people_applicant_wrtn_test_dtl_upd_submit ");
					action = "people_applicant_wrtn_test_dtl_upd_submit";
				}
			}
			else
...
if (action.equals("upd_wrtn_record"))// selecting particular record  for HR interview
			{
				...
				for( int s=0; s < dummyApplicantTestDtlList.size(); s++)
				{
					applicantTestDtlDBObj = (ApplicantTestDtlDBObj)dummyApplicantTestDtlList.get(s);// it has only all records always
					session.setAttribute("applicantTestDtlDBObj",applicantTestDtlDBObj);
				}
				// Till Here applicantTestDtlDBObj = (ApplicantTestDtlDBObj)dummyApplicantTestDtlList.get(0);// it has only one record always
			}
...
// New Addition 
		...
		if (action.equals("people_applicant_wrtn_test_dtl_upd_submit")){
			String dbopr = "";
		...
		if (action.equals("people_applicant_wrtn_test_dtl_submit")) // for all written,technical and HR detail
		{
		...
		if( action.equals("applicant_select_for_final_submit")){   
			...
			if ( applicantTestList != null && applicantTestList.size() > 0 ){
				for (int lsize =1; lsize <= applicantTestList.size() ;lsize++ ){
					...
					applicantTestDtlDBObj.applicant_id = listApplicantTestDtlDBObj.applicant_id;
					applicantTestDtlDBObj.applicant_name = listApplicantTestDtlDBObj.applicant_name;
					applicantTestDtlDBObj.test_date = listApplicantTestDtlDBObj.test_date;
					applicantTestDtlDBObj.test_time = listApplicantTestDtlDBObj.test_time;
					applicantTestDtlDBObj.present_status = "";
...
// New Added 
//			session.setAttribute("dbopr","upd_wrtn_performance");
		...
		if (action.equals("upd_applicant_wrtn_performance") || action.equals("upd_applicant_tech_performance") || action.equals("upd_applicant_hr_performance")){        
			...
			if (action.equals("upd_applicant_tech_performance")){
				criteria = "where test_status='T' and applicant_id not in (select applicant_id from APPLICANT_TEST_DETAIL where test_status in ('HR','confirm'))";	
// New Added				
//				session.setAttribute("dbopr","upd_tech_performance");
			}
			else
			if (action.equals("upd_applicant_hr_performance")){ 
				criteria = "where test_status='HR' and applicant_id not in (select applicant_id from APPLICANT_TEST_DETAIL where test_status in ('confirm'))";
// New Added		
				//session.setAttribute("dbopr","upd_hr_performance");
			}
			else
		...
		if (action.equals("people_applicant_call")){
			String lApplicantId = "";
			lApplicantId = (String)request.getParameter("applicant_id");
...
// Newly added
		...
		if (action.equals("people_applicant_remove")){        
			String lApplicantId = "";
			lApplicantId = (String)request.getParameter("applicant_id");
			ArrayList prevApplicantTestList = new ArrayList();
			prevApplicantTestList = (ArrayList)session.getAttribute("applicantTestList");
